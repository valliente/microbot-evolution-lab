export function sanitizePath(relativePath: string): string {
  ...
  if (window.location.href.includes('android_asset') || window.location.origin === 'https://localhost' || window.location.protocol === 'file:' || window.location.protocol === 'capacitor:' || window.location.protocol === 'ionic:') {
...
export function resolveShaderUrl(shaderPath: string): string {
  ...
  if (resolved.startsWith('./') || resolved.startsWith('blob:')) return resolved;
